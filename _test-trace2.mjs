...
async function run() {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (opt) {
      ...
      const ni = page.locator('button').filter({ hasText: 'I have no idea' });
      if (await ni.count() > 0) { await ni.click(); await page.waitForTimeout(300); const ys = page.locator('button').filter({ hasText: 'Yes, show me' }); if (await ys.count() > 0) await ys.click(); await page.waitForTimeout(300); }
    }
